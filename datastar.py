...
class Expr(ABC):
    ...
    def toggle_in(self, value: Any) -> "Assignment":
        ...
        sig = self.to_js()
        return Assignment(self, _JSRaw(
            f"{sig}.includes({val}) ? {sig}.filter(v => v !== {val}) : [...{sig}, {val}]"
        ))
...
class Signal(Expr):
    ...
    @property
    def id(self) -> str:
        """Returns the signal's identifier string (name without $ prefix).

        Use this when you need the clean signal name as a string:
            data_bind=my_signal.id
            data_ref=my_signal.id
            Progress(signal=my_signal.id)
        """
...
def seq(*exprs: Any) -> _JSRaw:
    """Sequential evaluation using comma operator: seq(a, b, c) → (a, b, c).

    Evaluates all expressions left-to-right and returns the last value.
    Useful for executing multiple side effects in event handlers.
    """
    if not exprs:

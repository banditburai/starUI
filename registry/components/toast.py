...
class ToastHelper:
    ...
    def __call__(
        ...
        auto_dismiss = set_timeout(js("$toasts=$toasts.map(x=>x&&x.id===t.id?null:x)"), duration) if duration else ""
        core = (
        ...
        return f"(()=>{{const fn=()=>{{{core}}};document.startViewTransition?document.startViewTransition(fn):fn()}})()"

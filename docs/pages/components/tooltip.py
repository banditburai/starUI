...
from starhtml import Div, P, Span, Icon, Label, Code, Strong, H3
...
def examples():
    """Generate tooltip examples using ComponentPreview with tabs."""
    ...
    yield ComponentPreview(
        ...
        title="Basic Tooltip",
        description="A simple tooltip that appears on hover with default positioning"
    )
    ...
    def dynamic_content_example():
        return Div(
            ...
            Tooltip(
                TooltipTrigger(
                    Button(
                        ...
                        ds_on_click("$copied = true; setTimeout(() => $copied = false, 2000)"),
                        variant="outline",
                        size="icon"
                    ),
                    delay_duration=300
                ...
                TooltipContent(
                    Span(ds_text("$copied ? 'Copied!' : 'Click to copy'"))
                ),
                signal="tooltip_copy"
            ),
            # Click counter
            Tooltip(
                ...
                TooltipContent(
                    Span("Clicked ", Span(ds_text("$clicks"), cls="font-bold"), " times")
                ),
                signal="tooltip_counter"
            ),
            ds_signals(copied=False, clicks=0),
            cls="flex gap-4 justify-center"
        )
...
def create_tooltip_docs():
    ...
    # API Reference
    # For Tooltip, users need to understand the building blocks (sub-components)
